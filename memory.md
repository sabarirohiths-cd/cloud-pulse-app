...
- **Service Standardization:** Fixed explicit imports for AWSService (pp.services.aws.service) due to an empty __init__.py structure.

#### Frontend Modernization & Routing
- **Unified Navigation:** Configured eact-router in App.js to provide seamless navigation between the Config, Inventory, and Control modules under a single responsive shell.
- **Tailwind Integration:** Established a consistent, modern UI using Tailwind CSS, moving away from disparate CSS files.

#### UI Virtualization
- **DOM Freeze Prevention:** Replaced standard HTML tables in the Control module (ResourcesTab.js) with eact-virtuoso (TableVirtuoso). This ensures the UI remains silky smooth even when interacting with datasets of 8,000+ cloud resources by only rendering rows currently visible on the screen.
...
- **Summary Aggregation APIs:** Created a standalone /summary endpoint in the Control module that uses raw SQL unc.sum() and case() logic to calculate total running/stopped counts instantaneously without transferring actual records to the frontend.

#### Control Page Refactoring & Infinite Scroll
- **Architectural Shift:** Completely removed the global esources state array from ControlPage.js. The parent component now only manages global filters and lightweight summary data.
...
- **FastAPI Path Matching:** Refactored pi/control.py state polling to use {resource_id:path} to support ARNs containing slashes.

### July 29, 2026 (Database Locks & Aggressive Scale-to-Zero)
- **Database Transaction Lock Bypass:** Updated pi/control.py and pi/inventory.py to explicitly release SQLite locks using wait db.commit() prior to long-running AWS API network scans. This solved a severe issue where a 4-minute background AWS network scan froze all other API calls attempting to access the dashboard.
...
- **Delayed Skeleton Loaders:** Fixed aggressive UI flickering on fast data fetches by wrapping TableSkeleton and header spinners in a delayed ade-in animation (150ms). This prevents loading indicators from flashing if the backend responds near-instantaneously.
...
- **Background TTL Cleanup:** Injected a garbage collection routine into the un_control_scheduler background loop. It runs exactly once per hour to automatically purge any system notifications older than 48 hours, preventing long-term SQLite database bloat.
