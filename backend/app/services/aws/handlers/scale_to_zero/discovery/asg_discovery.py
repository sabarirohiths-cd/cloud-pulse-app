...
async def async_find_parent_instance_from_asg(asg_name: str, session) -> str:
    try:
        async with session.client('autoscaling') as autoscaling, session.client('ec2') as ec2:
            ...
            if 'LaunchTemplate' in asg:
                ...
                lc_name = asg['LaunchConfigurationName']
                lc_res = await autoscaling.describe_launch_configurations(
                    LaunchConfigurationNames=[lc_name]
                )
                lcs = lc_res.get('LaunchConfigurations', [])
                if lcs:
            ...
            img_res = await ec2.describe_images(ImageIds=[image_id])
            images = img_res.get('Images', [])
            if not images:
                return None
            ...
            snap_res = await ec2.describe_snapshots(SnapshotIds=[snapshot_id])
            snapshots = snap_res.get('Snapshots', [])
            if not snapshots:
                return None
