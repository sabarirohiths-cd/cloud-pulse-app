...
async def async_discover_asg_and_cp_status(session, cluster_name: str, service_name: str = None) -> Tuple[bool, list, list]:
    has_managed_cp = False
    managed_asgs = []
    unmanaged_asgs = []
    ...
    async with session.client('ecs') as ecs:
        ...
        if service_name:
            try:
                res = await ecs.describe_services(cluster=cluster_name, services=[service_name])
                if res.get('services'):
                    service = res['services'][0]
                    target_cp_names = [cp.get('capacityProvider') for cp in service.get('capacityProviderStrategy', []) if cp.get('capacityProvider')]
            except Exception:
        ...
        try:
            ...
            if clusters:
                ...
                if target_cp_names:
                    cp_res = await ecs.describe_capacity_providers(capacityProviders=target_cp_names)
                    for cp in cp_res.get('capacityProviders', []):
                        asg_config = cp.get('autoScalingGroupProvider', {})
                        if asg_config.get('managedScaling', {}).get('status') == 'ENABLED':
                            has_managed_cp = True
        ...
        try:
            ci_arns = []
            if service_name:
                task_res = await ecs.list_tasks(cluster=cluster_name, serviceName=service_name)
                task_arns = task_res.get('taskArns', [])
                if task_arns:
                    desc_tasks = await ecs.describe_tasks(cluster=cluster_name, tasks=task_arns[:100])
                    for task in desc_tasks.get('tasks', []):
                        ci_arn = task.get('containerInstanceArn')
                        if ci_arn and ci_arn not in ci_arns:
                            ci_arns.append(ci_arn)
